...
const TicketComponent = dynamic(() =>
  import("../../components/patient-ticket/TicketComponent.js")
);
...
function AllTicketList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setRemoteData(
        json.data.filter((item) => item?.therapist_info?.id == therapistId)
      );
      setIsLoading(false);
    };
    fetchData();
  }, [remoteData, token]);
  console.log("check local stroge data ", remoteData);
  //     const json = await response.json();
  ...
  return (
    <>
      <main className="p-6  space-y-6">
        <TicketComponent title="Therapist ticket" />
